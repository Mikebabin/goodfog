...
- Theme: dark is the base palette on `:root` in `frontend/src/app.css`; light mode overrides the same tokens under `prefers-color-scheme: light` (no manual toggle). Every colour is a token; `src/lib/theme.test.js` rejects raw hex/rgba in component `<style>` blocks and keeps the two palettes' token sets identical. Map dot fills are the one deliberate exception (`scoreColor`, with `textColorFor` for contrast).
